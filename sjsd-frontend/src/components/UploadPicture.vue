    ...
    <div>
      ...
      >    <!-- visible.sync控制模态框的显示和隐藏 和一个变量进行绑定-->   <!-- before-close在关闭模态框前执行  和一个函数进行绑定 -->
        <el-upload
          ...
          <!-- el-icon-plus 类通常用于显示一个加号图标   avatar-uploader-icon是我自己设置的一个css-->
        </el-upload>
  ...
  export default {
    ...
    methods: {
      ...
      submitUpload() {
        ...
        //如果想上传用户上传过的所有图片，那么可以采用这几行代码
        // if (this.fileList.length > 0) {
        //     const finalFile = this.fileList[0].raw; // 直接使用 fileList 中的第一个文件
        //     this.$emit('avatar-uploaded', finalFile); // 触发事件，通知父组件头像已上传
        //     this.$message({
        //         showClose: true,
        //         message: '成功更改头像',
